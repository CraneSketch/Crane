...
def path_query(qw_model, path_edges):
    ...
    """
    return qw_model.query_path(path_edges)
...
def subgraph_query(qw_model, subgraph_edges):
    ...
    """
    return qw_model.query_subgraph(subgraph_edges)
...
def _load_downstream_queries(task_dir, device):
    ...
    if not os.path.isfile(npz_path):
        raise FileNotFoundError(
            f"Real downstream queries not found: {npz_path}. "
            f"Generate the workload with the companion Artifact dataset builders."
        )
...
def evaluate_downstream(cfg, qw_model, metric_logger):
    """Evaluate stored path-flow and subgraph-flow workloads."""
    ...
    for task_path in cfg.eval.dataset_path_list:
        ...
        topk_list = list(cfg.downstream.get("topk_list", [100]))
        for suffix, preds, targets in (("PathQuery", path_preds, path_targets),
                                       ("SubGraphQuery", sg_preds, sg_targets)):
            rows = [build_metric_row(dataset.task_names[i], pred, targets[i],
                                     memory_dims[i], topk_list, cfg.get("seed", ""))
                    for i, pred in enumerate(preds)]
            metric_logger.log_downstream_final(f"{dataset.name}_{suffix}", rows)
