...
from crane.utils import setup_logger, make_time_directories, load_merged_cfg, set_seed, weight_scale_for
from crane.eval import eval_one_task


logger = logging.getLogger("crane")
NODEFLOW_TASKS = ("NodeFlowOut", "NodeFlowIn")
...
def _load_single_npz(npz_path, task_type, device, weight_scale=1.0):
    with np.load(npz_path, allow_pickle=False) as data:
        support_x = torch.as_tensor(data["support_x"], device=device)
        support_y = torch.as_tensor(data["support_y"], dtype=torch.float32,
        ...
        if task_type == "Basic":
            ...
            query_y = torch.as_tensor(data["query_edge_y"], dtype=torch.float32,
                                      device=device).reshape(-1) * weight_scale
        elif task_type == "Node":
            query_x = torch.as_tensor(data["query_node_x"], device=device)
            query_y = torch.as_tensor(data["query_node_y"], dtype=torch.float32,
                                      device=device).reshape(-1) * weight_scale
        elif task_type in NODEFLOW_TASKS:
            key = "query_out_y" if task_type == "NodeFlowOut" else "query_in_y"
            query_x = torch.as_tensor(data["query_node_x"], device=device)
            query_y = torch.as_tensor(data[key], dtype=torch.float32,
                                      device=device).reshape(-1) * weight_scale
        else:
...
def main(cfg: DictConfig):
    ...
    for dataset_path in cfg.eval.dataset_path_list:
        ...
        weight_scale = weight_scale_for(cfg, dataset_name)
        subdirs = _scan_and_sort_task_dirs(dataset_path)

        for subdir in subdirs:
            subdir_path = os.path.join(dataset_path, subdir)
            train_npz = os.path.join(subdir_path, "finetune_train.npz")
            val_npz = os.path.join(subdir_path, "finetune_val.npz")
            if not os.path.isfile(val_npz):
                val_npz = os.path.join(subdir_path, "finetune_test.npz")
            eval_npz = os.path.join(subdir_path, "0.npz")

            if not os.path.isfile(train_npz) or not os.path.isfile(val_npz):
            ...
            train_item = _load_single_npz(train_npz, cfg.task_type, cfg.device, weight_scale)
            val_item = _load_single_npz(val_npz, cfg.task_type, cfg.device, weight_scale)
            ...
            finetune(
                ...
                cache_written_memory=cfg.finetune.get("cache_written_memory", False),
                select_by=cfg.finetune.get("select_by", "val_loss"),
            )

            if not cfg.finetune.get("evaluate_after", True):
            ...
            eval_item = _load_single_npz(eval_npz, cfg.task_type,
                                         cfg.eval.data_device, weight_scale)
