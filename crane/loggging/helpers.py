...
class MetricLogger(object):
    def __init__(self, root_path, source_code_dir=None, task_type="Basic"):
        ...
        self.downstream_path = os.path.join(root_path, "downstream_metric.csv")
        self._downstream_columns = None
    ...
    def log_downstream_final(self, dataset_name, rows):
        if not rows:
            return
        columns = ["dataset", *rows[0].keys()]
        if self._downstream_columns is None:
            self._downstream_columns = columns
            with open(self.downstream_path, "w") as f:
                f.write(",".join(columns) + "\n")
        elif columns != self._downstream_columns:
            raise ValueError("downstream metric columns changed")
        with open(self.downstream_path, "a") as f:
            for row in rows:
                values = [dataset_name, *(value.item() if isinstance(value, torch.Tensor) else value
