#!/usr/bin/env python3
"""Build ForCrane tasks from the CAIDA 2018 passive trace."""
import argparse
import os
import time
...
DATASET = "CAIDA2018"
RAW_FILE = "CAIDA2018.dat"
PAPER_SIZES = [2_000_000, 8_000_000, 16_000_000, 30_000_000]
NODE_BITS = 32
...
def read_edges(path, limit):
    ...
    with open(path, "rb") as stream:
        buffer = stream.read(record_len * limit)
    count = len(buffer) // record_len
    records = np.frombuffer(buffer, dtype=np.uint8,
                            count=count * record_len).reshape(count, record_len)
    src = np.ascontiguousarray(records[:, :4]).view(">u4").reshape(-1).astype(np.int64)
    dst = np.ascontiguousarray(records[:, 4:8]).view(">u4").reshape(-1).astype(np.int64)
    log(f"read {count} records")
    return src, dst
